...
public class BaseDaoProxy<T> implements InvocationHandler {
  ...
  private BaseDaoObj dealInfluxdbHql(Method method, Object arg) throws Exception {
    ...
    //父类泛型的名称
    String measurementSimpleName = superActualTypeArgument.getSimpleName();
    //泛型的包路径
    String measurementPackagePath = superActualTypeArgument.getName();
    logger.debug("[superClassGeneric]superClassGenericSimpleName:{},superClassGenericPackagePath:{}"
        ,measurementSimpleName,measurementPackagePath);

    //根据泛型的measurement的注解获取database和measurement
    Measurement measurementAnnotation = (Measurement) superActualTypeArgument.getAnnotation(Measurement.class);
    String measurementName = measurementAnnotation.name();
    String measurementDatabase = measurementAnnotation.database();

    logger.info("[superClassGeneric]measurementName:{},measurementDatabase:{}",measurementName,measurementDatabase);
    ...
    baseDaoObj.setClassPath(measurementPackagePath);
    baseDaoObj.setMeasurementClass(superActualTypeArgument);
    baseDaoObj.setMeasurementClassName(measurementSimpleName);
    return baseDaoObj;
  }
